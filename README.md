...
- **2026-05-24 — E1-LLM Route A-ter: the online bound is structural, not semantic.** Renaming the death-tile `lava` → `pit` (a name whose LLM prior — "you fall in, it's terminal" — *coincides* with the world's rule) was the pre-registered test of two sub-causes from Route A-bis: (i) semantic mismatch of the prior on "lava", vs (ii) a structural *wisdom-vs-prediction* confound in the online per-query world-model. **Renaming does not recover entry-detection** — balanced f_abs only nudges from 0.54 to **0.59** (threshold was 0.80); pooled death stays at **39.4%**, 0/6 layouts significant. The LLM refuses to predict an agent *entering* a dangerous tile *regardless of the name* — it conflates "what happens IF the action is taken" (world-model) with "what the agent SHOULD do" (judgment). The offline-holds / online-fails boundary of the FMC+LLM merge is **structural** to per-query NL inference. **Route A is concluded** (three pre-registered variants exhausted). Surviving constructive paths (out of scope): an open domain where LLM priors *match* the rules, or a perception-organ that labels tiles operationally *before* the world-model. See [`work/12_conjecture_e/E1_LLM_ROUTE_A_TER_RESULT.md`](work/12_conjecture_e/E1_LLM_ROUTE_A_TER_RESULT.md).
- **2026-05-22 — E1-LLM Route A & Route A-bis: the merge holds offline, not online.** With the LLM world-model queried *online* per-step from local observations (no rules given), self-preservation collapses — pooled death ~35–39% vs **0/180** for the offline code-form world-model. Route A-bis tested and **falsified** the first diagnosis (persistence): enforcing the absorbing invariant in the framework changed nothing. The balanced probe reveals the real cause — the online LLM's absorbing fidelity is **f_abs ≈ 0.54, at the chance floor**: without the rules, it models lava with the wrong prior ("obstacle to avoid", not "lethal terminal tile"). Code-form (Route B) worked because the rules were *given and transcribed*. The FMC+LLM merge is bounded to the offline regime. See [`work/12_conjecture_e/E1_LLM_ROUTE_A_BIS_RESULT.md`](work/12_conjecture_e/E1_LLM_ROUTE_A_BIS_RESULT.md).
...
- Canonical math is now [`docs/MATH_CANON.md`](docs/MATH_CANON.md) **v0.7.8** — 6 definitions, 3 theorems, **5 falsifiable conjectures (A–E)**, each with empirical-verification status.
