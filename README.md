...
- **2026-05-22 — E1-LLM Route A: the merge holds offline, not online.** With the LLM world-model queried *online* per-step from local observations (no rules given), self-preservation collapses — pooled death **35%** vs **0/180** for the offline code-form world-model. The cause is pinpointed: of the three fidelity axes, entry-detection (0.92) and movement (0.94) hold, but **absorbing-persistence collapses to 0.53** — queried per-step, the LLM does not keep terminal states terminal, an invariant Route B's *code* enforced structurally. The FMC+LLM merge is bounded to the offline regime; the forward fix is to enforce persistence in the framework. See [`work/12_conjecture_e/E1_LLM_ROUTE_A_RESULT.md`](work/12_conjecture_e/E1_LLM_ROUTE_A_RESULT.md).
