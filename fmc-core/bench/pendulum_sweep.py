"""Pendulum alpha x beta sweep — third task for Conjecture A (Bet 3).

Task with fundamentally different reward landscape from rocket/navigation:
energy-based (cos(theta)) rather than position-based.

Usage:
    python -m bench.pendulum_sweep
    python -m bench.pendulum_sweep --quick
    python -m bench.pendulum_sweep --full
"""
...
def main():
    ...
    parser.add_argument(
        "--output",
        default=str(Path(__file__).parent / "results" / "pendulum_sweep.jsonl"),
    )
    ...
    for alpha, beta in cells:
        ...
        notes = f"pendulum alpha={alpha} beta={beta} (Bet 3 third task — energy-based reward)"
        result = run_cell(
            benchmark="pendulum_alpha_beta_sweep",
            env_name="pendulum_swingup_K9",
            params=params,
            metric="b_eff",
