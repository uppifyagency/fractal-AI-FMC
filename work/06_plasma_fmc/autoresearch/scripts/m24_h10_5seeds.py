...
def make_exp39_controller(sim_p, seed: int = 0):
    ...
    ctrl = FMCPlasmaJaxController(
        sim_p,
        n_walkers=2048,
        horizon=10,  # M22 mutation: H=10 → 15
        dt=1e-3,
        voltage_std=80.0,
        P_aux=1e6,
        gas_puff=2e21,
    ...
    ctrl._weights = jnp.array(
        [640.0, 640.0, 320.0, 320.0], dtype=DTYPE,
    )
    return ctrl
...
def main():
    ...
    # Targets: ALL 6 M15 published + M16 real = 7-target full oracle baseline
    targets = {"M16_real_TCV_65402": TCV_X21_TARGET}
    for s in PUBLISHED_TARGETS:
        targets[f"M15_{s.name}_steady"] = np.array(
            s.waypoints[-1][1], dtype=np.float32)
    ...
    n_ticks = 30
    n_seeds = 5  # keep small for oracle expense
    ...
    # Save
    out_path = OUT_DIR / "m24_h10_5seeds_oracle.json"
