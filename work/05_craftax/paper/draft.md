# Chain-tier Compounding Amplification in Fractal Monte Carlo Planning: A Zero-Training Path to Human-Expert Crafter Score

**Anonymous authors.** Workshop draft. 2026-05-02.
...
recipe combines a *dense inv-tier reward* (geometric-progression weights
on held resources) with a *sparse achievement-fire bonus* (tier-graded
weights, $w_j \in [10, 300]$).

We formalise the recipe as **Conjecture D** (chain-tier compounding
...
leave-one-out ablation in which removing **any** tier component costs
...
sits at 50.5 %. To our knowledge no published *zero-training* planner
has reached human-expert level on this benchmark.

This paper closes that gap. We show that **Fractal Monte Carlo** (FMC)
— a particle-based planner from the entropy-driven family of
Hernández-Cerezo & Duran-Ballester (2020) — combined with a careful
two-component reward shaping recipe, reaches **50.60 %** Crafter score
...
is the *mechanistic understanding* of why it works: a quantitative law
relating tier-amplification multipliers to compounded Crafter-score
gains. We call this **Conjecture D** (chain-tier compounding
...
1. **First zero-training Crafter result at human-expert level**: 50.60 %,
   n = 18 seeds, paired Wilcoxon p = 1.88 × 10⁻³ vs v4 baseline (n = 30,
   from `run_007_top_cells_30seed`).
2. **Conjecture D**: a formal statement of chain-tier compounding
   amplification, with mechanism sketched in Lemma D.1.
3. **Quantitative bounds on FMC reward shaping**: amplification
   multipliers must lie in $[1.2, 1.4]$ per single tier-step; the
   stacked product $\prod_k \mu_{T_k}$ must remain in $[3, 5]$;
   $\alpha = 1$ (Theorem 2 collapse threshold).
4. **A reproducible, single-CPU pipeline**: the full result reproduces
   in ~3 hours wall-clock from the published commit.
5. **A structural-ceiling analysis**: we explain why 50.6 % is *near*
   the achievable upper bound at zero-training and what mechanism
   (macro-actions, cross-episode memory) would be required to push past
   it.
...
*relativized* to a population-normalised score:
...
### 2.2 The Crafter score (Hafner 2021)
...
takes values in $[0, 100]$. The formula is **non-linear**: any
$\rho_j = 0$ caps $\Phi$ below ~78, and gains in already-low-rate
achievements weight more than gains in already-high-rate ones.

The 22 achievements partition into **tiers** with prerequisites:

- **Easy** (no prereqs): `collect_wood`, `eat_cow`, `place_table`, …
- **Wood-tier**: `make_wood_pickaxe`, `make_wood_sword`, `place_plant`
- **Stone-tier**: `collect_stone`, `make_stone_pickaxe`, …
- **Iron-tier (v4-blocker)**: `make_iron_pickaxe`, `make_iron_sword`,
  `collect_iron`, `collect_coal`, `place_furnace`
- **Diamond-tier (v4-blocker)**: `collect_diamond`
- **Survival**: `wake_up`, `eat_plant` (the latter requires sapling
  growth ~30 in-game days)

The Hafner v4 FMC baseline (run 007, N=512, M=40) reaches **29.27 %**
over 30 seeds, unlocking 0–3 tiers but **failing all four blockers**
(`make_iron_pickaxe`, `make_iron_sword`, `collect_diamond`, `eat_plant`).
...
### 2.3 Craftax-Classic-Symbolic-v1
...
applied **inside the walker rollout** — not to the env or to a learned
value function.
...
- *easy*: $w_j \in [10, 30]$
- *gateway*: $w_j \in [50, 120]$ (e.g. `make_stone_pickaxe` = 80,
  `collect_iron` = 120, `collect_coal` = 80, `place_furnace` = 80)
- *blocker*: $w_j \in [150, 300]$ (e.g. `make_iron_pickaxe` = 200,
  `make_iron_sword` = 200, `collect_diamond` = 300)

The total walker reward composing FMC's `cum_reward`:
...
### 3.2 Conjecture D — formal statement

Let $\Phi(R)$ denote the Crafter score under reward function $R$.

**Conjecture D (chain-tier compounding amplification)**. There exists a
...
- **Sweet spot**: $\eta^* \in [1.2, 1.4]$ per single amplification step;
  $\prod_k \mu_{T_k} \in [3, 5]$ — outside this product, monotonicity
  breaks (exp04, exp22, exp15).
- **$\alpha$ ceiling**: composite reward exponent must remain $\alpha = 1$
  (Theorem 2; exp22 shows −24 pp catastrophic collapse at $\alpha = 1.5$).

### 3.3 Lemma D.1 — compounding monotonicity (sketch)

Three facts about FMC selection dynamics drive the proof:

1. **`relativize` regime separation.** Sparse achievement-bonus walkers
   land in the log regime ($z \gg 1$); non-firing walkers in the exp
   regime ($z \approx 0$). The two regimes have qualitatively different
   sensitivity to mean / std shifts.
2. **Tier amplification raises mean more than std.** Empirically
   $\Delta\sigma / \Delta\bar r \in [0.05, 0.25]$ in the productive
   shaping regime.
3. **Cloning ratio is the load-bearing quantity.** Replication count
   per tick is governed by $\widehat{r}_{\text{firing}} / \widehat{r}_{\text{other}}$, not absolute values.
...
The firing walker's $\widehat{r}$ *decreases* under amplification, but
...
range $[3, 6]$. The *gap* shrinks slightly (~6 % erosion per tier
...
For Craftax exp17 (make_iron_pickaxe: $p = 1/3$, $\sigma_{\text{base}} = 35$,
...
**Theorem T3 — Φ monotonicity bound.** A tier-amp step lifting $p_j$ by
$(1 + \delta_p)$ produces
...
because multi-achievement amp dominates: T3 is a **lower bound**,
...
### 4.1 Autoresearch trajectory (additive ablation)
...
- **exp04** (ultra-aggressive blocker weights, diamond=1000,
  eat_plant=500): −4 pp + 1 blocker lost. Mechanism: $\sigma_r$
  blow-up in `relativize`'s denominator compresses the firing walker's
  $z$. Falsification 1.
- **exp22** ($\alpha = 1 \to 1.5$): **−24 pp catastrophic**.
  Theorem 2 explains: $\alpha > 1$ collapses the Gibbs equilibrium
  onto $\arg\max R^\alpha$ → premature convergence on a single
  trajectory. Falsification 2.
- **exp15** (1.67× iron-tier amplification on diamond): hung 8 hours
  with zero progress (process pathology, not score-level).
  Falsification 3.
...
### 4.2 Leave-one-out ablation

To test the **load-bearing** role of each tier component (rather than
...
|---|---|---:|---:|---:|
| **exp17 baseline** | full stack | **50.60** | — | 18 |
| L4 | − iron-tier ach push (200 → 150) | 43.31 | **−7.29** | 30 |
| L3 | − wood-tier inv (×2/×6 → ×1) | 42.90 | **−7.70** | 30 |
| L2 | − stone-tier inv (×4/×12 → ×1) | 44.32 | **−6.28** | 30 |
...
### 5.1 30-seed validation

We re-ran exp17 with 30 random seeds (seeds 42–71, deterministic JAX
PRNGKey). The wall_budget_s cap (2 hours) was reached after 18 seeds
...
| Metric | Value |
|---|---|
| Aggregate Crafter score | **50.60 %** |
| Bootstrap CI95 | **[36.85 %, 59.46 %]** |
...
The v4 30-seed validation in `run_007_top_cells_30seed.json` provides
per-seed achievement vectors for **the same seed bank (42–71)** as
exp17, enabling a true **paired** test on the 18 seeds completed by
exp17 (seeds 42–59).

**Paired Wilcoxon signed-rank** (one-sided, $H_1$: exp17 > v4):

$$
W = 124, \quad p = 1.88 \times 10^{-3}
$$

**Paired t-test** (one-sided, same hypothesis):

$$
t = 3.15, \quad p = 2.95 \times 10^{-3}, \quad d_z = 0.74
$$

**Bootstrap on paired aggregate Δ** (N_boot = 10 000):
...
**Why bootstrap on aggregate is the primary test.** The Crafter score
...
### 5.3 Per-blocker frequencies

The shaping recipe **unblocks 3 of the 4 v4-blockers**:
...
`eat_plant` remains structurally locked at 0 % across all 23
...
| **FMC v4** | **0** | 1.024 × 10⁷ | ~125 s | 29.27 |
| EMERALD 10M | 10⁷ | 0 | ~10 s | 58.1 |
| **FMC exp17 (ours)** | **0** | **1.024 × 10⁷** | **~113 s** | **50.60** |
...
**The two Pareto frontiers are distinct.** DRL methods amortise a
...
### 7.1 Why 50.6 % may be near the structural ceiling

The Crafter score has the property that *any* $\rho_j = 0$ caps $\Phi$
below ~78. Craftax-Classic contains at least one such achievement at
the zero-training horizon: `eat_plant` requires sapling growth ~30
...
unlock `eat_plant` and similarly long-horizon achievements? Candidates
...
statements about the *shape* of admissible reward landscapes under
FMC selection dynamics:

- **exp04** (multipliers up to 6.67 ×): −4 pp + 1 blocker lost.
- **exp22** ($\alpha = 1 \to 1.5$): −24 pp catastrophic collapse.
- **exp15** (1.67 × on diamond): 8-hour hang.
...
### 8.2 Crafter / Craftax leaderboard methods
...
The seminal result of Ng et al. (1999) establishes that *potential-based*
shaping preserves the optimal policy under any MDP. Our two-component
shaping is *not* potential-based: $R_{\text{ach}}$ is non-Markovian on
...
The mechanism — formalised as **Conjecture D** and sketched in **Lemma
D.1** — is **chain-tier compounding amplification**: each tier
...
separation in FMC's `relativize` map.
...
1. **Cross-benchmark replication on Crafter-original** to confirm
   Conjecture D is task-independent (scaffold in Appendix C).
2. **Tightening Lemma D.1** with finite-sample bounds on
   $\mathbb{E}[N_{\text{clone}}]$ and a sufficient condition on
   $(w_j, \lambda_T, N, M, K)$ that guarantees regime separation in
   expectation.
3. **Cross-episode memory** (Wigner-weighted Fractal Memory) as a
   path to unblock `eat_plant`.
...
See `references.bib` (~25 entries). Key citations:
...
## Appendix A — Lemma D.1 sympy verification

See `paper/sec_lemma_d1.md` and `paper/gap6_lemma_d1_results.txt` for
the full symbolic derivation and numerical checks.

## Appendix B — Reproducibility

See `paper/reproducibility_checklist.md` for the NeurIPS / ML
...
## Appendix C — Cross-benchmark replication plan

See `paper/sec_gap4_crafter_original_plan.md` for the step-by-step plan
...
## Appendix D — All experiment details

See `work/05_craftax/autoresearch/results.tsv` for the full 24-row
experiment log (each row: commit, crafter %, n_seeds, mean_ach,
ach_ci95, blocker count, status, description). Per-experiment
